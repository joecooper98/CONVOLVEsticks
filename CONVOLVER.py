...
islorentzian = False  # set to True if you want a lorentzian
...
def eVtonm(e): # function that returns the nm value of an energy in eV
    nm=(299792458*4.135667516E-6)/(e)
    return nm
...
def eVtoinvcm(energy):
    return 8065.54429 * energy

def lorentzian(x, height, centre, width):
    gam=0.5*width
    l=1/(np.pi*gam) * gam / ((x-centre)**2 + gam**2)
    return l * height
...
spec[:,1] = [eVtonm(x) for x in spec[:,0]]
...
if islorentzian:
    ...
    for i in range(noex): # for all excitations, calculate a gaussian of height of the oscillator strength centred at the energy, and put into column n=4
        spec[:,i+4] = N * lorentzian(spec[:,0],data[i+noex],data[i],FWHM)

else :
    c = FWHMtoC(FWHM) # converts user input FWHM into the standard dev
    NonC = 11452.3149/c # L^3 mol^-1 cm ^ -1 = Na * e ^2 * h / (4*m_e*c*epsilon_0*ln(10)*sqrt(2pi) * c) # conversion factor you times by f/c
    for i in range(noex): # for all excitations, calculate a gaussian of height of the oscillator strength centred at the energy, and put into column n=4
        spec[:,i+4] = NonC * gauss(spec[:,0],data[i+noex],data[i],c)


spec[:,3] = [np.sum(spec[i,4:]) for i in range(samples)] # sum over all the columns to get total spectrum

np.savetxt("Spectrum_"+filename,spec,fmt='%.6e', delimiter='   ') # save the data.
